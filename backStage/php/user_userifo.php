...
if(isset($_SESSION['name'])){
	...
	if($res){
		if($res[4]=='1'){
			if($serach_key==''){
				$sql = "select * from login limit $page_start,$page_limit";
				$result = mysqli_query($link,$sql);	
				...
				if($res){
					for($i=0;$i<sizeof($res);$i++){
						...
						$arrayList[$i]['password'] = $res[$i][2];
						$arrayList[$i]['register_time'] = $res[$i][3];
						$arrayList[$i]['super_admin'] = $res[$i][4];
					}
			...
			//搜索
				...
				$sql = "select * from login where username like '%$serach_key%'";
				$result = mysqli_query($link,$sql);	
				$res = mysqli_fetch_all($result);
				if($res){
					for($i=0;$i<sizeof($res);$i++){
